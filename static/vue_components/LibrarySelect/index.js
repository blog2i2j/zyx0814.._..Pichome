...
const LibrarySelect = {
	props:{
		...
		defaultcheckedtype:{//默认选择类型
			required:false,
			type: Array,
			default:['unlimit','library','default','folder'],
		},
		isdefault:{
			required:false,
			type: Boolean,
	...
	template: `
		<el-popover
		    ...
		    :placement="placement"
			popper-class="LibrarySelect-popper"
				...
				<div class="el-cascader" 
					...
					<div class="el-input el-input--suffix " :class="{'is-focus':popovervisible,'is-disabled':disabled}">
						<div class="el-input__wrapper">
							<input type="text" readonly="readonly" :placeholder="Inputbox.data.length?'':placeholder" class="el-input__inner" :style="{height: Inputbox.height+'px'}">
	...
	methods:{
		async handlefilterText(val){
			var res = await axios.post('index.php?mod=pichome&op=librarySelect&do=search',{
				str:val
			});
			if(res == 'intercept'){
				return false;
		...
		InputboxDelete(data){
			...
			let id = data.id;
			let checkedData = [];
			if(self.$refs['Librarytree_'+this.defaulttype]){
				checkedData = self.$refs['Librarytree_'+this.defaulttype].getCheckedKeys();
				if(checkedData.length){
					self.$refs['Librarytree_'+this.defaulttype].setChecked(id);
				}
			}
			...
			this.$nextTick(() => {
				if(self.$refs['Librarytree_'+this.defaulttype]){
					...
					for(let i =0 ; i<checkedData.length;i++){
						if(checkedData[i].indexOf(data.id)>-1){
							self.$refs['Librarytree_'+this.defaulttype].setChecked(checkedData[i],false);
							continue;
						}
						newchekcedData.push(checkedData[i]);
					}

					this.Tree.checkeds = newchekcedData;
				}else{
		...
		TreeChange(data, node){
			...
			if(data.type == 'unlimit' && index_unlimit>-1){
				self.$refs['Librarytree_'+this.defaulttype].setCheckedKeys(['unlimit']);
				...
				if(index_unlimit>-1){
					self.$refs['Librarytree_'+this.defaulttype].setChecked('unlimit',false);
				}
			...
			if(newcheckedNodes.length){
				for (let index = 0; index < newcheckedNodes.length; index++) {
					...
					for(let i = 0; i < storageid.length ; i++){
						if(element.id.indexOf(storageid[i])>-1){
							b=1;
							break;
						}
					}
		...
		async TreeLoadNode(node, resolve){
			...
			if (node.level === 0) {
				...
				if(node.data.type == 'folder'){
					param.id=node.data.fid;
				}else{
					param.id = node.data.id;
				}
			}

			let res = await axios.post('index.php?mod=pichome&op=librarySelect&do=tree',param);
			if(res == 'intercept'){
				return false;
			}
			var json = res.data;
			...
			for (let index = 0; index < json.length; index++) {
				...
				item.leaf=item.isLeaf?true:false;
			...
			this.$nextTick(function(){
				if(self.$refs['Librarytree_'+this.defaulttype] && checkeddata.length){
					for (let index = 0; index < checkeddata.length; index++) {
						const element = checkeddata[index];
						self.$refs['Librarytree_'+this.defaulttype].setChecked(element,true);
					}
				}
			});
		},
		PopoverShow(){
			...
			self.$nextTick(function(){
				if(!self.Inputbox.data.length){
					self.$refs['Librarytree_'+this.defaulttype].setCheckedKeys([]);
				}
			});
