...
$start = ($i - 1) * $perpage;
$j = 0;
foreach(DB::fetch_all("select  count(id) as num,fid  from %t where 1  group by fid  limit $start,$perpage",['pichome_folderresources']) as $value){
    if($value['fid']){
        $filenum = $value['num'] ? $value['num']:0;
        C::t('pichome_folder')->update($value['fid'],['filenum'=>$filenum]);
    }
