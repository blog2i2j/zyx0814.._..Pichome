<?php
if(!isset($_GET['count'])){
    $count = DB::result_first("select count(DISTINCT rid) from %t where 1  ",['pichome_resources']);
}else{
...
$start = ($i - 1) * $perpage;
$j = 0;
foreach(DB::fetch_all("select  r.rid,GROUP_CONCAT(rf.fid) as fids from %t r left join %t rf on r.rid = rf.rid
                     where 1  group by r.rid  limit $start,$perpage",['pichome_resources','pichome_folderresources']) as $value){

    $fids = $value['fids'] ? $value['fids']:'';
    C::t('pichome_resources')->update($value['rid'],['fids'=>$fids]);
