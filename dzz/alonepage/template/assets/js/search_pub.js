const Tmpsearch_pub = {
    ...
    setup(props,context){
        ...
        function searchclassifyChange(data){
            ...
            if(data.length){
                for (let index = 0; index < data.length; index++) {
                    ...
                    for (let findex = 0; findex < DataList1.value.length; findex++) {
                        const felement = DataList1.value[findex];
        ...
        async function getSearchList(query){
            ...
            const {data: res} = await axios.post(BasicUrl+'getSearchList',{q: query,ids:props.model.data[0].data[0].searchclassify});
            if(res.success){
