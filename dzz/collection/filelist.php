...
if ($operation == 'filelist') {
    ...
    if($clid){
        ...
        if($perm < 1 && !$isshare) exit(json_encode($return));
    }

    $selectsql = " cl.rid,cl.id as lid ";
    $preparams = [];
    $havingsql = '';
    $havingparams = [];
    $sql = " from  %t cl left join %t r on cl.rid=r.rid ";
    ...
    if (isset($_GET['color'])) {
        ...
        $selectsql .= ",max(round((%d-sqrt((((2+(p.r+%d)/2)/256)*(pow((%d-p.r),2))+(4*pow((%d-p.g),2)) + (((2+(255-(p.r+%d)/2))/256))*(pow((%d-p.b), 2)))))/%d,4)*p.weight) as ow ";
        $preparams = array($maxColDist, $rgbcolor['r'], $rgbcolor['r'], $rgbcolor['g'], $rgbcolor['r'], $rgbcolor['b'], $maxColDist);
        $sql .= " left join %t p on r.rid = p.rid ";
        $params[] = 'pichome_palette';
        $havingsql .= ' having ow > %d';
        $havingparams[] = $similarity;
        $orderarr[] = ' ow desc';
    }
    ...
    foreach (DB::fetch_all(" select $selectsql $sql where $wheresql $groupby $havingsql order by $ordersql $limitsql", $params) as $value) {
       $resourcesdata = C::t('pichome_resources')->fetch_by_rid($value['rid'],$isshare);
       //$tmpdata = array_merge($value,$resourcesdata);
       //$data[$value['lid']] = $tmpdata;
        $lrids[$value['lid']] = $value['rid'];
      // $rids = $value['rid'];
    }

    $rids = array_values($lrids);
    ...
    foreach($rdata as $key=>$value){
       // $value['dpath'] = Pencode(array('path'=>$value['rid'],'perm'=>0,'ishare'=>0,'isadmin'=>1),3600);
        $resourcesdata[$key] = $value;
    }
    foreach($lrids as $k=>$v){
    ...
    $isdeleteappids = [];
    //查询断开链接和删除数据
    foreach(DB::fetch_all("select appid,path,isdelete,`type` from %t where appid in(%n)",array('pichome_vapp',$appids)) as $v){
        if($v['isdelete']) $isdeleteappids[] = $v['appid'];
        elseif($v['type'] != 3  &&  IO::checkfileexists($v['path'], 1)) $noconnectappids[] = $v['appid'];
    }
    $resultdata = [];
    foreach($data as $k=>$v){
        if(in_array($v['appid'],$isdeleteappids)) $v['isdelete'] = 1;
        elseif(in_array($v['appid'],$noconnectappids)) $v['noconnect'] = 1;
        else{
            $v['isdelete'] = 0;
            $v['noconnect'] = 0;
        }
    ...
    // if (count($resultdata) >= $perpage) {
    //     $total = $start + $perpage * 2 - 1;
    // } else {
    //     $total = $start + count($resultdata);
    // }
    ...
    $return = array(
        'total' => $total,
        'data' => $resultdata ? $resultdata : array(),
        'next'=>$next,
        'param' => array(
            'order' => $order,
            'page' => $page,
