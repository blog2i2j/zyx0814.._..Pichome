...
require_once DZZ_ROOT . './dzz/aliBaiLian/class/class_baiLian.php';
...
class ImagetagAnddesc
{
    ...
    public function run(&$return, $extar = [])
    {
        ...
        $promptdata = C::t('#aliBaiLian#bailian_imageprompt')->fetch($tplid);
        ...
        if ($imgurl) {
            ...
            if ($data = C::t('ai_imageparse')->insertData($setarr)) {
                ...
                if ($data['isget'] && $data['data'] && !$force) {
                    ...
                    $params['processname'] = $this->waitLock('DZZ_bailian_IAMGEPARSE');

                    $bailian = new \baiLian(null);
                    $aireturn = $bailian->getAiData($params);

                    if ($aireturn['code']){
                        $return=['error' => $aireturn['message']];
                        @unlink($imgurl);
                        return $return;
                    }
                    if($aireturn){
                        ...
                        C::t('ai_imageparse')->update($data['id'], ['isget'=>1,'data'=>$aireturn['content'],'totaltoken'=>$aireturn['totaltoken']]);
                         $content = $aireturn['content'];
                    }else{
                        C::t('ai_imageparse')->update($data['id'], ['isget'=>1,'data'=>'']);
                        $return=['error' => 'bailian::chatImage error'];
                        @unlink($imgurl);
                        return $return;
                    }
