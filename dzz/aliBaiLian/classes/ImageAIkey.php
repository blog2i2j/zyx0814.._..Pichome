...
class ImageAIkey
{
    public function run(&$data, $params = [])
    {
        ...
        $imageSetting = C::t('setting')->fetch('setting_bailian', true);
        ...
        if (($type == 'file' || $type == 'files')&& array_intersect($imagetypes, $data['exts'])) {
            ...
            if ($type == 'file') {
                if($imageSetting['chatModel']) {
                    $data['Aichat'][] = [
                        'url' => getglobal('siteurl') . 'index.php?mod=aliBaiLian&op=chat',
                        'recordurl' => getglobal('siteurl') . 'index.php?mod=aliBaiLian&op=chat&do=getHistory',
                        'delhistoryurl' => getglobal('siteurl') . 'index.php?mod=aliBaiLian&op=chat&do=clearchat',
                        'type' => 'image',
                        'params' => 'rid',
                        'name'=>lang('appname',array(),'','dzz/aliBaiLian'),
                        'key'=>'bailian::chatImage'
                    ];
                }
            }
    ...
    public function getAiKey(&$data,$type,$appdata){
        ...
        $promptdata = C::t('#aliBaiLian#bailian_imageprompt')->fetchPromptByStatus();
        $tplsdata = [];
