...
if ($do == 'getHistory') {//获取历史对话
    ...
    if ($type == 'image') {
        ...
        $historydaya = C::t('#aliBaiLian#bailian_chat')->fetchContentByIdvalue($idval, $idtype);

        $returndata = [];
        foreach ($historydaya as $k => $v) {
            $tmmpcontent = json_decode($v['content'], true);
            $content = '';
            if (is_array($tmmpcontent['content'])) {
                $content = $tmmpcontent['content'][1]['text'];
            } else {
    ...
    if ($type == 'image') {
        ...
        C::t('#aliBaiLian#bailian_chat')->deleteContentByIdvalue($idval, $idtype);
...
// 从 get 中获取提问
    ...
    if ($type == 'image') {
        ...
        //缺少图片id参数
        if (!$idval) {
            $bailian->end('Missing image id');
        } else {
            //获取对话id
            $metadata = IO::getMeta($idval);
            if (!$metadata) $bailian->end('file is not exists');
            else {
                $rid = $idval;
                if ($metadata['aid']) {
                    $idval = $metadata['aid'];
                ...
                if (!$imgurl) {
                    $bailian->end('file is not allowed');
                } else {
                    ...
                    $return1 = waitLock('DZZ_CHAT_' . $idval, $bailian);
                    $params['processname'] = $return1;

                    $ret = $bailian->getChatData($params);
                    if ($ret['code']) {
                        dzz_process::unlock($params['processname']);
                        $bailian->end($ret['message']);
                    }
                    @unlink($imgurl);
                }
...
function waitLock($processnameprefix, $bailian)
{
    $locked = true;
    for ($i = 0; $i < 2; $i++) {
