...
elseif ($operation == 'searchmenu_num') {
   ...
   if(!$isrecycle) $wheresql = " r.isdelete = 0 and r.level <= %d ";
    ...
    $stdata = C::t('intelligent')->fetch_by_tid($sid);
...
elseif($operation == 'getpagesetting'){
    ...
    $stdata = C::t('intelligent')->fetch($sid);
    ...
    $stdata = C::t('intelligent')->fetch_by_tid($sid);
    $appids=[];
    if(isset($stdata['searchRange']['appids'])) {
        $appids = array_keys($stdata['searchRange']['appids']);
    }
    if(empty($appids)){
        $appids=array('');
    }
    $pichomefilterfileds =!is_array($stdata['screen'])?json_decode($stdata['screen'],true):$stdata['screen'];

    $tagval = $_GET['tag'] ? explode(',',trim($_GET['tag'])):[];
    if(!empty($tagval)){
        foreach(DB::fetch_all("select t.tagname,t.tid,tr.cid from %t  t  left  join %t tr on t.tid = tr.tid and tr.appid in (%n) where t.tid in(%n) ",array('pichome_tag','pichome_tagrelation',$appids,$tagval)) as $tv){
                ...
                if(count($cids)){
                    if(in_array(-1,$cids)){
                        ...
                        foreach(DB::fetch_all("select distinct t.tid,t.tagname,vt.hots from %t vt 
                            ...
                            left join %t t on t.tid=vt.tid where isnull(tr.cid) and vt.appid IN(%n) %s ORDER BY vt.hots DESC $limitsql ",
...
elseif($operation == 'bannerlist'){ //获取栏目列表
    $tid = isset($_GET['tid']) ? trim($_GET['tid']) : '';
    $bannerdata = C::t('pichome_banner')->getBannerTree();
    $bannerlist=$bannerdata['bannerlist'];
    if($active=getActiveByBannerList($bannerlist['top'],$tid)){
        $bannerlist['active']=$active;
    }elseif($active=getActiveByBannerList($bannerlist['bottom'],$tid)){
        $bannerlist['active']=$active;
    }else{
        $bannerlist['active']=[];
    }
...
function getActiveByBannerList($list,$tid){

    foreach($list as $k=>$v){

        if($v['btype']=='1' && $v['bdata']==$tid){
        ...
        if($v['children']){
            if($ret  = getActiveByBannerList($v['children'],$tid)){
                return $ret;
            }
        }
    }
