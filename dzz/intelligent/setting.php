...
if($do=='create'){
	...
	$setarr=array(
		'title'=>$title,
		'exts'=>$_GET['exts'],
		'searchRange'=>str_replace('unlimit','',(is_array($_GET['searchRange'])?implode(',',$_GET['searchRange']):$_GET['searchRange'])),
		'dateline'=>TIMESTAMP,
		'disp'=>intval($_GET['disp']),
        'tags'=>is_array($_GET['tags'])?implode(',',$_GET['tags']):'',
    ...
    $extra=array();
    if(isset($_GET['grade'])){
        if(!in_array('0',$_GET['grade'])){
            $extra['grade']=dintval($_GET['grade'],true);
        }
    }

    if($_GET['searchName']){
        $extra['searchName']=htmlspecialchars($_GET['searchName']);
    }
    if($extra){
        $setarr['extra']=json_encode($extra);
    }else{
        $setarr['extra']='';
    }
	if($setarr['tid']=C::t('intelligent')->insert_by_tid($setarr)){
	...
	$tid=intval($_GET['tid']);
	if(C::t('intelligent')->delete_by_tid($tid)){
...
elseif($do=='fetch'){
    ...
    if (submitcheck('settingsubmit')) {
		...
		C::t('intelligent')->update_by_tid($tid,$setarr);
        $setarr['tid'] = $tid;

        exit(json_encode(array('success' => true)));
    } else {
        require_once(DZZ_ROOT . './dzz/class/class_encode.php');
        if ($data = DB::fetch_first("select * from %t where tid=%s ", array('intelligent', $tid))) {
            ...
            $data['searchRange']=C::t('intelligent')->getEditSearchRange($data['searchRange'],$data['tid']);
    ...
    if ($total = DB::result_first("select count(*) from %t where $sql", $params)) {
        foreach (DB::fetch_all("select * from %t where $sql order by disp,dateline desc limit $start,$perpage", $params) as $value) {
            ...
            if ($value['searchRange']) {
                $arr = C::t('intelligent')->getFidsBysearchRange($value['searchRange'], $value['tid']);
                $appnames = array();
                foreach ($arr['appids'] as $v) {
                    $appnames[] = $v['appname'];
                }
                foreach ($arr['folders'] as $v) {
                    $appnames[] = $v['appname'] . ':' . implode('-', $v['pathname']);
                }
    ...
    exit(json_encode(array('success' => true, 'data' => $data, 'total' => $total)));
