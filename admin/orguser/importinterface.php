...
if (!defined('IN_OAOOA') || !defined('IN_ADMIN') || !defined('PICHOME_LIENCE')) {
    ...
    global $_G;
    if($_G['adminid']!=1) {
        exit(json_encode(array('error'=>lang('orguser_import_user'))));
    }
    ...
    if($do=='importing'){
        ...
        if(empty($email) || empty($_GET['nickname'])) exit(json_encode(array('error'=>lang('name_email_empty'))));
        if(empty($_GET['username'])) $_GET['username']=$_GET['nickname'];
        ...
        //检查用户是否已经存在
        if(($user=C::t('user')->fetch_by_email($email)) || ($user=C::t('user')->fetch_by_nickname($_GET['nickname']))){//用户已经存在时
            $uid=$user['uid'];
            ...
            if($isappend){//增量添加，如果原先没有nickname,增加
                $appendfield=array();
                $appendfield['username']=$_GET['username'];
                ...
                if(!check_username($_GET['nickname'])) exit(json_encode(array('error'=>lang('user_name_sensitive'))));
            ...
            //验证用户限制
            if(!checkUserLimit()){
                exit(json_encode(array('error'=>lang('license_user_exceed'))));
            }
            if(!check_username($_GET['nickname'])) exit(json_encode(array('error'=>lang('user_name_sensitive'))));
            
            $result=uc_add_user($_GET['username'], $_GET['password'], $email,$_GET['nickname']);
            if (is_array($result)) {
                if($result['error']){
            ...
            if ($uid <= 0) {
                if ($uid == -1) {
                    exit(json_encode(array('error'=>lang('profile_nickname_illegal'))));
                } elseif ($uid == -2) {
                    exit(json_encode(array('error'=>lang('profile_nickname_protect'))));
                } elseif ($uid == -3) {
                    exit(json_encode(array('error'=>lang('profile_nickname_duplicate'))));
                } elseif ($uid == -4) {
                    exit(json_encode(array('error'=>lang('profile_email_illegal'))));
                } elseif ($uid == -5) {
                    exit(json_encode(array('error'=>lang('profile_email_domain_illegal'))));
                } elseif ($uid == -6) {
                    exit(json_encode(array('error'=>lang('profile_email_duplicate'))));
                } elseif ($uid == -7) {
                    exit(json_encode(array('error'=>lang('profile_nickname_illegal'))));
                } else {
        ...
        //处理部门和职位
        $orgids=(intval($_GET['orgid'])) ? [intval($_GET['orgid'])]:[];
        $orgdataarr = !empty($_GET['orgname']) ? explode(',',$_GET['orgname']):array();
        foreach($orgdataarr as $v){
            $orgname =!empty($v)?explode('/',$v):array();
            $orgid = 0;
            //$_GET['job']=!empty($_GET['job'])?explode('/',$_GET['job']):array();
            foreach( $orgname as  $key => $orgname){
      ...
      /*  if($orgid){
        ...
        $inputFileName = $_G['setting']['attachdir'].urldecode($_GET['file']);
        if(!is_file($inputFileName)){
            header("Location".ADMINSCRIPT.'?mod=orguser&op=importinterface');
        }
        ...
        //获取导入数据的字段
        $h0=array('nickname'=>lang('compellation'),'username'=>lang('username'),'email'=>lang('email'),'birth'=>lang('date_birth'),'gender'=>lang('gender'),'mobile'=>lang('cellphone'),'weixinid'=>lang('weixin'),'orgname'=>lang('category_department'),'job'=>lang('department_position'),'password'=>lang('user_login_password'));
        ...
        if(!in_array('username',$h)){
            exit(json_encode(array('error'=>lang('lack_required_fields_name'))));
        }elseif(!in_array('email',$h) && !in_array('username',$h)){
            exit(json_encode(array('error'=>lang('lack_required_fields_name_email'))));
        }
        if(!in_array('email',$h)){
        ...
        foreach($sheetData as $key=> $value){
            ...
            foreach($value as $col =>$val){
                ...
                if($h[$col]=='orgname'){
                    ...
                    $temp[$h[$col]]=dgmdate(strtotime($val),'Y-m-d');
                }else{
        ...
        exit(json_encode(array('orgid'=>$open,'headerflag'=>$h,'headername'=>$h0,'list'=>$list)));
    }else{
        if(submitcheck('importfilesubmit')){
            if($_FILES['importfile']['tmp_name']){
                ...
                if(!in_array($ext,$allowext)) exit(json_encode(array('error'=>lang('orguser_import_xls_xlsx'))));
                if($file=uploadtolocal($_FILES['importfile'],'cache','',array('xls','xlsx'))){
					exit(json_encode(array('success'=>true,'file'=>urlencode($file))));
					// cache/202011/26/221554pfk1dkh1o1tnf94f.xlsx
                    // $url=outputurl($_G['siteurl'].MOD_URL.'&op=importinterface&do=list&file='.urlencode($file));
                    // @header("Location: $url");
                    // exit();
                }else{
                    exit(json_encode(array('error'=>lang('orguser_import_tautology'))));
                }
            }else{
                exit(json_encode(array('error'=>lang('orguser_import_user_message_table'))));
            }
        }
    }
