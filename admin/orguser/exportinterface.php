...
if (!defined('IN_OAOOA') || !defined('IN_ADMIN') || !defined('PICHOME_LIENCE')) {
    ...
    $h0 = array('nickname' => lang('compellation'), 'username' => lang('username'), 'email' => lang('email'), 'birth' => lang('date_birth'), 'gender' => lang('gender'), 'mobile' => lang('cellphone'), 'weixinid' => lang('weixin'), 'orgname' => lang('category_department'), 'job' => lang('department_position'));
    $h1 = getProfileForImport();
    $h0 = array_merge($h0, $h1);
    
    $orgid = intval($_GET['orgid']);
    $uids = $_GET['uids'] ? trim($_GET['uids']) : '';
    ...
    $defaultitem = ($_GET['item']) ? $_GET['item'] : ['nickname', 'username', 'email', 'birth', 'gender', 'mobile', 'weixinid', 'orgname'];
    if (!is_array($defaultitem)) exit(json_encode(array('error' => lang('please_select_project_export'))));
    foreach ($h0 as $key => $value) {
        if (!in_array($key, $defaultitem)) unset($h0[$key]);
    }
    ...
    $objPHPExcel->getProperties()->setCreator($_G['username'])
        ->setTitle($title . ' - ' . lang('user_information_table') . ' - oaooa')
        ->setSubject($title . ' - ' . lang('user_information_table'))
        ->setDescription($title . ' - ' . lang('user_information_table') . ' Export By oaooa  ' . date('Y-m-d H:i:s'))
    ...
    foreach (DB::fetch_all("select * from %t $wheresql", array('user')) as $user) {
        ...
        if ($orgids = C::t('organization_user')->fetch_orgids_by_uid($value['uid'])) {
            ...
            foreach ($orgids as $key => $gid) {
                ...
                $value['orgnamearr'][] = str_replace('-', '/', $orgpath);
            }
            $value['orgname'] = implode(',', $value['orgnamearr']);
            unset($value['orgnamearr']);
            ...
            foreach ($h0 as $key1 => $fieldid) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($index, $value[$key1], PHPExcel_Cell_DataType::TYPE_STRING);
                $j++;
