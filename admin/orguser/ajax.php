...
//						$arr['text'] = avatar_group($value['orgid'],array($value['orgid']=>array('aid'=>$value['aid'],'orgname'=>$value['orgname']))).$value['orgname'];
	...
	if(!$org=C::t('organization')->fetch($orgid)){
		exit(json_encode(array('error'=>lang('institution_group_deleted'))));
	}
	//暂时只允许系统管理员进行空间相关设置
	if($_G['adminid'] != 1){
		exit(json_encode(array('error'=>lang('permission_denied'))));
	}
	if($setspacesize != 0){
		...
		if($allowallotspace < 0) {
			exit(json_encode(array('error' => lang('Insufficient_allocate'))));
		}
		...
		if($setspacesize > 0 && $setspacesize*1024*1024 < $currentallotspace){
			...
			exit(json_encode(array('error'=>lang('Insufficient_value'),'val'=>$org['maxspacesize'])));

		}
		//上级包含空间限制时，无限制不处理，直接更改设置值
		if($allowallotspace > 0 && ($setspacesize*1024*1024 > $allowallotspace)){

			exit(json_encode(array('error'=>lang('Insufficient_space'),'val'=>$org['maxspacesize'])));
	...
	if(C::t('organization')->update($orgid,array('maxspacesize'=>$setspacesize))){
		...
		exit(json_encode(array('error'=>lang('settings_unsuccessful'),'val'=>$org['maxspacesize'])));
	}
}elseif ($do == 'getroles') {
	$roledata=array();
	 foreach (Hook::listen('getRoles',$temp,null,true) as $v) {
        $roledata[] = array('text' => $v['desc'], 'id' => $v['id']);
    }
    exit(json_encode(array('total_count' => count($roledata), 'items' => $roledata)));
}elseif ($do == 'guide') {
